import React, { useState } from "react";

function ContactMe() {
  const [emailData, setEmailData] = useState({
    name: "",
    email: "",
    subject: "",
    body: "",
  });

  const handleChange = (e, inp) => {
    setEmailData({ ...emailData, [inp]: e.target.value });
  };

  const sendEmail = () => {
    alert(JSON.stringify(emailData, null, 2));
  };

  return (
    <div>
      <h1 className="font-sans text-5xl m-10">My Socials</h1>
      <div className="border-2 border-solid" />
      <div className="grid grid-cols-2 gap-x-1 place-items-center w-full">
        <div
          className="w-1/4 border-2 border-solid m-5"
          style={{ minHeight: "80vh" }}
        >
          <div className="m-4 text-2xl text-blue-500">Social Media</div>
        </div>
        <div
          className="w-3/4 border-2 border-solid  m-5"
          style={{ minHeight: "80vh" }}
        >
          <div className="m-4 text-2xl text-blue-500">Email Me!</div>
          <div className="border-2 border-solid p-4 m-4 flex flex-col mb-4">
            {Object.keys(emailData).map((inp) => (
              <>
                <label className="ml-5">{inp.toLocaleUpperCase()}</label>
                <input
                  name={inp}
                  onChange={(e) => handleChange(e, inp)}
                  className="border-2 border-solid border-gray-400 m-4"
                  key={inp}
                />
              </>
            ))}
            <button
              className="border-2 border-solid block bg-blue-300 hover:bg-blue-400 text-black uppercase text-lg mx-auto p-4 rounded-md"
              onClick={() => sendEmail()}
            >
              Send Email
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ContactMe;
